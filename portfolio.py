...
def VaR(hist_port,c=0.95):
    rets=pd.DataFrame(hist_port).pct_change()
    mu = np.mean(rets)
    sigma = np.std(rets)
    P=1
...
class PortfolioSim(object):
    ...
    def _step(self, w1, y1):
        ...
        #get VaR
        self.hist_port_val.append(p1)
        risk = VaR(self.hist_port_val)
...
#        self.observation_space = np.concatenate(np.ones((1, self.window_length, history.shape[-1])),gym.spaces.Box(low=-np.inf, high=np.inf, shape=(len(abbreviation), window_length,
...
#        return self._step(action)
    ...
    def step(self, action):
        ...
        """
#        print(action.shape,len(self.sim.asset_names))
...
#        risk = self.risk()
        ...
        return observation, (reward,risk), done1 or done2, info
...
#            df_info[["portfolio_value", "market_value"]].plot(title=title, fig=plt.gcf(), rot=30)
    ...
    def is_early_episode_termination(self, cost=None, time_steps=None, total_cost=None):
        if time_steps > self.max_time_steps and total_cost<0:
            return True, 5
        elif time_steps > self.max_time_steps and total_cost>0:
            return True, 0.
        else:
            return False, 0.  
